...
resource "aws_kms_key" "nm_backup_vault_kms" {
    description         = "KMS CMK for AWS Backup Vault"
    customer_managed_key_spec = "SYMMETRIC_DEFAULT"
    is_enabled          = true
...
resource "aws_kms_alias" "nm_backup_vault_kms_cmk" {
    name        = "alias/nm_cmk_kms"
    target_key_id   = aws_kms_key.nm_backup_vault_kms.key_id
}
